...
def extract_text(file_path, extension):
    ...
    if extension.endswith('.pdf'):
        for page in extract_text_from_pdf(file_path):
            text += ' ' + page
    elif extension.endswith('.docx') or extension.endswith('.doc'):
...
def get_total_experience(experience_list):
    ...
    for line in experience_list:
        experience = re.search(
            '(?P<fmonth>\w+.\d+)\s*(\D|to)\s*(?P<smonth>\w+.\d+|present)', line, re.I)
        if experience:
    ...
    for phrase in text_split:
        if len(phrase) == 1:
            ...
            p_key = set(phrase.lower().split()) & set(
                cs.RESUME_SECTIONS_PROFESSIONAL)
        try:
            p_key = list(p_key)[0]
        except IndexError:
            pass
...
def extract_measurable_results(text, experience_list):
    ...
    experience_text = ' '.join([text[:len(text) // 2 - 1]
                                for text in experience_list])
    mr_dict = {}
        ...
        for item in cs.MEASURABLE_RESULTS[mr]:
            if string_found(item, experience_text):
                if mr not in mr_dict.keys():
                    match = re.search(
                        r'([^.|,]*' + item + '[^.|,]*)', experience_text_for_matching)
                    if item not in matches.keys():
                        matches[item] = [match.group(0)]
                    else:
                        for i in match.groups():
